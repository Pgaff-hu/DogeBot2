...
const starts = async (dogexeon = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('PGA BOT', {
		font: 'Standard',
	...
	console.log(color('\n> YT CHANNEL: PGA FF ','silver'))
console.log(color('> GITHUB: PGA-FF ','silver'))
console.log(color('> WA NUMBER: +918617738028 ','silver'))
console.log(color('  PGA Bot Inc. 2021','mediumseagreen'))
	dogexeon.browserDescription = [ 'Subscribe PGA FF', 'ubuntu', '3.0' ]
...
dogexeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "918617738028@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `PGA-BOT , img,but = [{buttonId:`getdescgc`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `PGA-BOT`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `GOODBYE !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
